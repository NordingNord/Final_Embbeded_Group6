...
extern "C" void infer(__cosim_sC__*, __cosim_sC__*);
extern "C" void apatb_infer_hw(volatile void * __xlx_apatb_param_infer_input, volatile void * __xlx_apatb_param_infer_output) {
  ...
  vector<__cosim_sC__> __xlx_infer_input_tmp_vec;
  while (!((hls::stream<__cosim_sC__>*)__xlx_apatb_param_infer_input)->empty() && __xlx_infer_input_V_tmp_Count < __xlx_infer_input_V_read_Size) {
    __xlx_infer_input_tmp_vec.push_back(((hls::stream<__cosim_sC__>*)__xlx_apatb_param_infer_input)->read());
    __xlx_infer_input_V_tmp_Count++;
  }
  ap_apatb_infer_input_V_cap_bc = __xlx_infer_input_tmp_vec.size();
  // store input buffer
  __cosim_sC__* __xlx_infer_input_input_buffer= new __cosim_sC__[__xlx_infer_input_tmp_vec.size()];
  ...
  __cosim_sC__* __xlx_infer_output_input_buffer= new __cosim_sC__[ap_apatb_infer_output_V_cap_bc];
  // DUT call
  infer(__xlx_infer_input_input_buffer, __xlx_infer_output_input_buffer);
  for (unsigned i = 0; i <ap_apatb_infer_output_V_cap_bc; ++i)
    ((hls::stream<__cosim_sC__>*)__xlx_apatb_param_infer_output)->write(__xlx_infer_output_input_buffer[i]);
}
